...
def lda_predict(answer_counters,message_dict,label_dict,model_file):
...
def lda_predict_nlp(answer_counters,message_dict,label_dict,model_file,cluster_info,glove,hybrid,pred_vectors,embeddings):
    predictions_to_write = []
    data_to_write = {}
    model = joblib.load(model_file)
    ...
    if embeddings == "bert":
        pred_vectors = transform_bert_for_lda(pred_vectors)
    if embeddings == "glove":
        ...
        pred_vectors = [transform_for_lda(test_vector) for test_vector in test_vecs]
